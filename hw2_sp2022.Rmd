...
4. PCA on 10 esteem measurements. (centered but no scaling)
    ...
    > The PCs are meant to be orthogonal to each other.
    
```{r}
(data.esteem.pc <- prcomp(data.esteem, center = T, scale. = F))
knitr::kable(data.esteem.pc$x[1:5,])

data.esteem.scores <- data.esteem.pc$x %>% as.data.frame()

summary(lm(PC2~PC1,data = data.esteem.scores))

summary(data.esteem.pc)$importance 
...
```
    ...
    Loadings are helpful in determining how much each variable contributes to the PCs.So taking PC1, we can see that the esteem variables that are higher are weight more. As an example Esteem87_1 is weighted at .226, while Esteem87_10 is weighted at .382, meaning that you should place more weight onto Esteem87_10. PC2 provides the second most variability and can implies that these self-esteem measures are characterized into two dimensions which are Esteem87_1 through Esteem87_7 and Esteem87_8 through Esteem87_10 
    ...
    PC1 = Estee87_1*0.226 + Estee87_2*0.239 + Estee87_3*0.280 + Estee87_4*0.251 + Estee87_5*0.307 + Estee87_6*0.317 + Estee87_7*0.301 + Estee87_8*0.395 + Estee87_9* 0.403 + Estee87_10* 0.382
    
    d) Are PC1 scores and PC2 scores in the data uncorrelated? 
    
    Yes. PC1 and PC2 are uncorrelated
    
    e) Plot PVE (Proportion of Variance Explained) and summarize the plot. 
    
    The plot shown above gives us an idea of how much the variance can be explained by each PC 
...
```{r}
factoextra::fviz_nbclust(data.esteem, kmeans, method = "wss") # a) Find a reasonable number of clusters using within sum of squared with elbow rules.

data.esteem.kmeans_02 <- kmeans(data.esteem, centers = 2)

data.esteem.kmeans_03 <- kmeans(data.esteem, centers = 3)

  
  #b) Can you summarize common features within each cluster?
str(data.esteem.kmeans_02)

str(data.esteem.kmeans_03)

data.esteem.kmeans_02$cluster 
data.esteem.kmeans_03$cluster
  ...
  data.frame(pc1 = data.esteem.pc$x[, 1], 
             pc2 = data.esteem.pc$x[, 2],
             group = as.factor(data.esteem.kmeans_03$cluster)) %>%
  ggplot(aes(x = pc1, y = pc2, col = group)) +
  geom_point() +
  ggtitle("Clustering over PC1 and PC2") 
...
(data.asvab.scores <- data.asvab.pc$x %>% as.data.frame())

ASVAB <- data.asvab.scores$PC1 %>% as.data.frame()

self_esteem <- data.esteem.scores$PC1 %>%  as.data.frame()
...
# Education Metrics
summary(lm(self_esteem~Education05,data = esteem))
summary(lm(self_esteem~MotherEd,data = esteem))
summary(lm(self_esteem~FatherEd,data = esteem))
summary(lm(self_esteem~ASVAB,data = esteem))
summary(lm(self_esteem~Ilibrary,data = esteem))
summary(lm(self_esteem~Inewspaper,data = esteem))
summary(lm(self_esteem~Imagazine,data = esteem))

summary(lm(self_esteem ~ Education05 + MotherEd + FatherEd + ASVAB + Ilibrary + Inewspaper + Imagazine,data = esteem))
...
brca_sub_standard <- scale(as.matrix(brca_sub), center = T, scale = T)

## Applying the irlba function instead of prcomp
brca_ret <- irlba::irlba(brca_sub_standard, nv = 10)
#names(brca_ret)

## Figuring out how many PCs we need
brca_var <- brca_ret$d^2/(nrow(brca_sub_standard)-1)
brca_apx <- brca_var/numgenes
plot(brca_apx, type="b", pch = 19, frame = FALSE)

# Possibly 2 or 3 or 4 PCs maybe be good based on the Plot Lets scave those scores
pc_score <- (brca_ret$u[, 1:4])*(brca_ret$d[1:4])

## Centering Data but not Scaling
brca_sub_standard_unscaled <- scale(as.matrix(brca_sub), center = T, scale = F)

## Applying the irlba function instead of prcomp
brca_ret_unscaled <- irlba::irlba(brca_sub_standard_unscaled, nv = 10)
#names(brca_ret_unscaled)

## Figuring out how many PCs we need
brca_var_unscaled <- brca_ret_unscaled$d^2/(nrow(brca_sub_standard_unscaled)-1)
brca_apx_unscaled <- brca_var_unscaled/numgenes

plot(brca_apx_unscaled, type="b", pch = 19, frame = FALSE)

# Possibly 4 PCs maybe be good based on the Plot Lets scave those scores
pc_score_unscaled <- (brca_ret_unscaled$u[, 1:4])*(brca_ret_unscaled$d[1:4])
...
4. Spectrum clustering: center but do not scale the data
    ...
    Seems like four is a good sweet spot
...
```
    ...
    4 Clusters
    Decent job for basal but awful for the other three subtypes
...
p_unscaled_centered <- data.table(x = pc_score_unscaled[,1], 
  ...
  scale_color_manual(values = scales::hue_pal()(4)) +
  theme_bw() +
  geom_point(aes(x = kmean_ret_unscaled$centers[1,1], y = kmean_ret_unscaled$centers[1,2]), size = 3, fill = "black", shape = 19) +
  geom_point(aes(x = kmean_ret_unscaled$centers[2,1], y = kmean_ret_unscaled$centers[2,2]), size = 3, fill = "black", shape = 17) +
  geom_point(aes(x = kmean_ret_unscaled$centers[3,1], y = kmean_ret_unscaled$centers[3,2]), size = 3, fill = "black", shape = 15) +
    geom_point(aes(x = kmean_ret_unscaled$centers[4,1], y = kmean_ret_unscaled$centers[4,2]), size = 3, fill = "black", shape = 3) +
  labs(color = "Cancer type", shape = "Cluster") +
  xlab("PC1") +
  ylab("PC2")
p_unscaled_centered
...
```
    ...
    PCA does a really good job if you look at the WithinSS. As you can see when applying PCA, the WithinSS is much smaller comparatively to the original data set. 
    
```{r}
list(c("WithinSS for original kmeans",brca_sub_kmeans$withinss),c("WithinSS for PCA",kmean_ret_unscaled$withinss))
```
...
## First Scaling and Centering the Data
x_patient_sub_standard <- t(scale(as.matrix(x_patient_sub[1,]), center = T, scale = F))


as.vector(x_patient_sub_standard)  %*% brca_ret$v[, 1]


 brca_ret$v[, 1:4]

## Applying the irlba function instead of prcomp
x_patient_ret <- irlba::irlba(x_patient_sub_standard,nv = 1)

# Possibly 2 or 3 or 4 PCs maybe be good based on the Plot Lets scave those scores
pc_score_x_patient <- (x_patient_ret$u[, 1:4])*(x_patient_ret$d[1:4])
