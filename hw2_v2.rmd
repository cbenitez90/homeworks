...
```{r}
capture <- which(!(results$lower_ci <= 1.3 & 1.3 <= results$upper_ci))
...
for(j in 1:100) {
  ...
  lines(c(results$lower_ci[j], results$upper_ci[j]), 
        c(j, j), 
        col = colors[j], 
        lwd = 2)
...
5 of our intervals do not capture the regression coefficient. This means 95% of the intervals are captured, which was expected given the 95% confidence interval. 
